...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract the keys (emails) where the value is `true` (i.e., permission is granted)
    const expandedEditPermissions = Object.keys(editPermissions)
    .filter(email => editPermissions[email]) // Only include emails where the permission is `true`
    .flatMap(email => [email, hashEmail(email)]); // Include both email and hashed email

    const expandedViewPermissions = Object.keys(viewPermissions)
    .filter(email => viewPermissions[email]) // Only include emails where the permission is `true`
    .flatMap(email => [email, hashEmail(email)]); // Include both email and hashed email
    ...
    const newChatCommand = new PutCommand({
      ...
      Item: {
        ...
        editors: expandedEditPermissions, // Handle edit permissions
        viewers: expandedViewPermissions, // Handle view permissions
        timestamp,
